...
public class BoardController {
    ...
    @GetMapping("/{page}")
    public String page(Model model, @RequestParam(value = "page", required = false, defaultValue = "1") Integer page) {
        List<Board> items = boardService.findAll();
        int totalPages = items.size() / 6 + (items.size() % 6 == 0 ? 0 : 1);
        int currentPage = page;

        model.addAttribute("items", items.subList((page - 1) * 6, Math.min(page * 6, items.size())));
    ...
    public String showView(BoardForm boardForm, @PathVariable Integer no, Model model){
        ...
        Iterable<Board> contlist = boardService.selectAll();
